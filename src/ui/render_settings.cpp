...
// --- App::renderSystemPromptPopup ----------------------------------

// Get system_prompt.md path (next to config.toml in exe directory)
static std::string getSystemPromptPath() {
    char exePath[MAX_PATH];
    GetModuleFileNameA(nullptr, exePath, MAX_PATH);
    fs::path p(exePath);
    return (p.parent_path() / "projv_prompts" / "system_prompt.md").string();
}

void App::renderSystemPromptPopup() {
    bool open = true;
    if (!ImGui::BeginPopupModal("System Prompt", &open, ImGuiWindowFlags_AlwaysAutoResize))
        return;

    ImGui::Text("Edit System Prompt (saved to system_prompt.md):");
    ImGui::Separator();
    ImGui::InputTextMultiline("##sysprompt", systemPromptBuf, sizeof(systemPromptBuf),
        ImVec2(560, 280));

    ImGui::Spacing();
    ImGui::Separator();
    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1),
        "Tip: system_prompt.md lives in projv_prompts/. You can also edit it directly.");

    if (ImGui::Button("Save && Apply")) {
        std::string prompt = systemPromptBuf;
        prompt.erase(0, prompt.find_first_not_of(" \t\n\r"));
        if (!prompt.empty()) {
            // Write to file (ensure directory exists)
            std::string path = getSystemPromptPath();
            fs::create_directories(fs::path(path).parent_path());
            std::ofstream ofs(path);
            if (ofs) {
                ofs << prompt;
                debugLogf("[App] Saved system prompt to %s (%zu chars)", path.c_str(), prompt.size());
            } else {
                debugLogf("[App] Failed to write system prompt to %s", path.c_str());
            }
            ...
            agent->addPersistedMessage(Message::System(prompt));
            ImGui::CloseCurrentPopup();
        }
    }
    ImGui::SameLine();
