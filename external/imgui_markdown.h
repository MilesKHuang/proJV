...
namespace ImGui
{
				...
				if( link.state == Link::NO_LINK && !line.isHeading )
                {
					...
					if( ( c == '*' || c == '_' )
                        && ( i == line.lineStart
                            || markdown_[ prev ] == ' '
                            || markdown_[ prev ] == '\t'
                            || !((markdown_[ prev ] >= 'a' && markdown_[ prev ] <= 'z')
                                || (markdown_[ prev ] >= 'A' && markdown_[ prev ] <= 'Z')
                                || (markdown_[ prev ] >= '0' && markdown_[ prev ] <= '9')) ) // also allow after punctuation
    ...
    inline void defaultMarkdownFormatCallback( const MarkdownFormatInfo& markdownFormatInfo_, bool start_ )
    {
		...
		case MarkdownFormatType::EMPHASIS:
        {
            ...
            {
                // strong emphasis — gold color (even without custom font)
    ...
    // ===================================================================
    // proJV Extensions — preprocessing, table rendering, unified entry
    // ===================================================================

    enum ProJVMarkdownFlags_
    {
        ProJVMarkdownFlags_None              = 0,
        ProJVMarkdownFlags_BacktickPairs     = 1 << 0,  // `code` -> _code_
        ProJVMarkdownFlags_DashRule          = 1 << 1,  // --- -> ___
        ProJVMarkdownFlags_RenderTables      = 1 << 2,  // Parse and render | tables
        ProJVMarkdownFlags_All = ProJVMarkdownFlags_BacktickPairs
            | ProJVMarkdownFlags_DashRule
            | ProJVMarkdownFlags_RenderTables,
    };

    // Pre-process markdown string: backtick pairs and horizontal-rule dashes
    inline std::string ProJVPreprocess(const std::string& input, int flags)
    {
        std::string md = input;
        if (flags & ProJVMarkdownFlags_BacktickPairs)
        {
            size_t bp = 0;
            while (bp < md.size()) {
                auto bt = md.find('`', bp);
                if (bt == std::string::npos) break;
                auto ct = md.find('`', bt + 1);
                if (ct == std::string::npos) break;
                md[bt] = '_'; md[ct] = '_';
                bp = ct + 1;
            }
        }
        if (flags & ProJVMarkdownFlags_DashRule)
        {
            size_t lp = 0;
            while (lp < md.size()) {
                auto nl = md.find('\n', lp);
                size_t lend = (nl == std::string::npos) ? md.size() : nl;
                std::string line = md.substr(lp, lend - lp);
                auto ns = line.find_first_not_of(" \t\r");
                if (ns != std::string::npos
                    && line.find_first_not_of("- \t\r", ns) == std::string::npos
                    && line.find("---") != std::string::npos)
                {
                    for (size_t li = lp; li < lend; ++li)
                        if (md[li] == '-') md[li] = '_';
                }
    ...
    // Render a tab-separated table via ImGui Table API.
    // Returns true if any rows were rendered.
    inline bool ProJVRenderTable(const std::string& content, const MarkdownConfig& mdConfig_)
    {
        struct TblRow { std::vector<std::string> cells; };
        std::vector<TblRow> tblRows;
        size_t tp = 0;
        while (tp < content.size()) {
            auto nl = content.find('\n', tp);
            std::string line = (nl == std::string::npos)
                ? content.substr(tp) : content.substr(tp, nl - tp);
            if (!line.empty()) {
                TblRow row;
                size_t cp = 0;
                while (cp < line.size()) {
                    auto tab = line.find('\t', cp);
                    row.cells.push_back((tab == std::string::npos)
                        ? line.substr(cp) : line.substr(cp, tab - cp));
                    cp = (tab == std::string::npos) ? line.size() : tab + 1;
                }
                tblRows.push_back(row);
            }
            if (nl == std::string::npos) break;
            tp = nl + 1;
        }
        if (tblRows.empty()) return false;

        int maxCols = 0;
        for (auto& r : tblRows)
            if ((int)r.cells.size() > maxCols) maxCols = (int)r.cells.size();
        maxCols = std::max(1, maxCols);

        std::vector<float> colWeights(maxCols, 8.0f);
        for (auto& r : tblRows)
            for (int ci = 0; ci < maxCols && ci < (int)r.cells.size(); ++ci) {
                float txtW = CalcTextSize(r.cells[ci].c_str()).x
                    + GetStyle().FramePadding.x * 2.0f;
                if (txtW > colWeights[ci]) colWeights[ci] = txtW;
            }

        ImGui::Separator();
        static int s_tblIdCounter = 0;
        char tblId[32];
        std::snprintf(tblId, sizeof(tblId), "pjv_tbl_%d", s_tblIdCounter++);

        if (ImGui::BeginTable(tblId, maxCols,
            ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg, ImVec2(-1, 0)))
        {
            for (int ci = 0; ci < maxCols; ++ci)
                ImGui::TableSetupColumn(nullptr,
                    ImGuiTableColumnFlags_WidthStretch, colWeights[ci]);
            for (auto& r : tblRows) {
                ImGui::TableNextRow();
                for (int ci = 0; ci < maxCols; ++ci) {
                    ImGui::TableNextColumn();
                    std::string cell = (ci < (int)r.cells.size()) ? r.cells[ci] : "";
                    if (!cell.empty()) {
                        cell = ProJVPreprocess(cell, ProJVMarkdownFlags_BacktickPairs);
                        ImGui::Markdown(cell.c_str(), cell.size(), mdConfig_);
                    }
                }
            }
            ImGui::EndTable();
    ...
    // Split text into table / non-table segments, pre-process, and render each.
    // Convenience wrapper that replaces manual preprocessing + table splitting in app code.
    inline void MarkdownEx(const char* markdown_, size_t markdownLength_,
        const MarkdownConfig& mdConfig_, int preprocessFlags = ProJVMarkdownFlags_All)
    {
        std::string text(markdown_, markdownLength_);

        // Pre-process
        if (preprocessFlags & (ProJVMarkdownFlags_BacktickPairs | ProJVMarkdownFlags_DashRule))
            text = ProJVPreprocess(text, preprocessFlags);

        // If no table rendering, just pass through
        if (!(preprocessFlags & ProJVMarkdownFlags_RenderTables)) {
            ImGui::Markdown(text.c_str(), text.size(), mdConfig_);
            return;
        }

        // Split into table / non-table segments
        struct TSeg { std::string content; bool isTable; };
        std::vector<TSeg> tsegs;
        size_t tp = 0;

        while (tp < text.size()) {
            size_t segStart = tp;
            auto tstart = text.size();

            // Find next line with ≥2 pipe characters
            while (tp < text.size()) {
                auto nl = text.find('\n', tp);
                size_t lend = (nl == std::string::npos) ? text.size() : nl;
                auto p1 = text.find('|', tp);
                auto p2 = (p1 == std::string::npos) ? std::string::npos
                    : text.find('|', p1 + 1);
                if (p2 != std::string::npos && p2 < lend) { tstart = tp; break; }
                if (nl == std::string::npos) { tp = text.size(); break; }
                tp = nl + 1;
            }
            if (tp >= text.size()) {
                if (segStart < text.size())
                    tsegs.push_back({text.substr(segStart), false});
                break;
            }

            if (tstart > segStart)
                tsegs.push_back({text.substr(segStart, tstart - segStart), false});

            // Collect table rows
            struct TblRow2 { std::vector<std::string> cells; };
            std::vector<TblRow2> tblRows;
            size_t rp = tstart;
            while (rp < text.size()) {
                auto rnl = text.find('\n', rp);
                size_t rlend = (rnl == std::string::npos) ? text.size() : rnl;
                auto pp1 = text.find('|', rp);
                auto pp2 = (pp1 == std::string::npos) ? std::string::npos
                    : text.find('|', pp1 + 1);
                if (pp2 == std::string::npos || pp2 >= rlend) break;

                std::string raw = text.substr(rp, rlend - rp);
                bool isSep = (raw.find("---") != std::string::npos)
                          || (raw.find("___") != std::string::npos);
                TblRow2 row;
                size_t cs = rp;
                if (cs < rlend && text[cs] == '|') ++cs;
                while (cs < rlend) {
                    auto pipe = text.find('|', cs);
                    if (pipe == std::string::npos || pipe >= rlend) {
                        std::string last = text.substr(cs, rlend - cs);
                        auto ss = last.find_first_not_of(" \t\r");
                        auto ee = last.find_last_not_of(" \t\r");
                        if (ss != std::string::npos) last = last.substr(ss, ee - ss + 1);
                        if (!last.empty() || !row.cells.empty() || isSep)
                            row.cells.push_back(last);
                        break;
                    }
                    std::string cell = text.substr(cs, pipe - cs);
                    auto ss = cell.find_first_not_of(" \t\r");
                    auto ee = cell.find_last_not_of(" \t\r");
                    if (ss != std::string::npos) cell = cell.substr(ss, ee - ss + 1);
                    row.cells.push_back(cell);
                    cs = pipe + 1;
                }
                if (!isSep) tblRows.push_back(row);
                rp = rnl + 1;
            }

            // Serialize collected rows as tab-delimited
            std::string tblStr;
            ...
            if (!tblStr.empty())
                tsegs.push_back({tblStr, true});
            tp = rp;
        }

        if (tp < text.size())
            tsegs.push_back({text.substr(tp), false});

        // Render all segments
        for (auto& ts : tsegs) {
            if (ts.isTable)
                ProJVRenderTable(ts.content, mdConfig_);
            else if (!ts.content.empty())
                ImGui::Markdown(ts.content.c_str(), ts.content.size(), mdConfig_);
        }
    }
