...
AI 在 TODO 面板实时管理任务清单，面板停靠在右侧（`F9`）并对长任务软换行，不会被截断。任务完成后不只是打个勾 -- **它会写一个结构化总结**：改了哪些文件、做了什么、编译验证结果。再也不用问"Agent 刚刚到底干了啥"。
...
| **角色主题分级** | 泡泡文字与 Agent 状态跟随主题语义色；AI 回复标题为 `proJV` |
| **忙碌指示** | 流式生成、执行工具、等待审批时都有滚动执行图标 |
